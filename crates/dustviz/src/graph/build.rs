...
impl Graph {
    ...
    pub fn add_node(&mut self, kind: NodeKind) -> NodeId {
        self.add_node_with_attrs(kind, Vec::new())
    }

    pub fn add_node_with_attrs(&mut self, kind: NodeKind, attrs: Vec<Attr>) -> NodeId {
        let id = self.nodes.len() as NodeId;
        self.nodes.push(Node { id, kind, attrs });
        id
    }

    pub fn add_edge(&mut self, kind: EdgeKind, from: NodeId, to: NodeId) -> EdgeId {
        self.add_edge_with_attrs(kind, from, to, Vec::new())
    }
...
pub fn build_dir_graph(program: &DirProgram) -> Graph {
    ...
    let program_id = g.add_node_with_attrs(NodeKind::Program, vec![Attr::new("kind", "program")]);
...
fn build_forge(g: &mut Graph, parent: NodeId, forge: &DirForge) -> NodeId {
    let forge_id = g.add_node_with_attrs(
        NodeKind::Forge {
            name: forge.name.clone(),
        },
        vec![
            Attr::new("kind", "forge"),
            Attr::new("name", forge.name.clone()),
        ],
    );
...
fn build_shape(g: &mut Graph, _forge_id: NodeId, shape: &DirShape) -> NodeId {
    g.add_node_with_attrs(
        ...
        vec![
            Attr::new("kind", "shape"),
            Attr::new("name", shape.name.clone()),
            Attr::new("field_count", shape.fields.len().to_string()),
        ],
    )
}

fn build_proc(g: &mut Graph, forge_id: NodeId, proc_: &DirProc) -> NodeId {
    let proc_id = g.add_node_with_attrs(
        NodeKind::Proc {
        ...
        vec![
            Attr::new("kind", "proc"),
            Attr::new("name", proc_.name.clone()),
            Attr::new("regime", proc_.regime.clone()),
            Attr::new("param_count", proc_.params.len().to_string()),
            Attr::new("uses_count", proc_.uses.len().to_string()),
            Attr::new("stmt_count", proc_.body.len().to_string()),
            Attr::new(
                "ret",
                proc_.ret.clone().unwrap_or_else(|| "null".to_string()),
            ),
            Attr::new("qualifier_count", proc_.qualifiers.len().to_string()),
        ],
    );
    ...
    for (idx, stmt) in proc_.body.iter().enumerate() {
        let stmt_id = build_stmt(g, proc_id, idx, stmt);
...
fn build_uses(g: &mut Graph, _proc_id: NodeId, uses: &DirUses) -> NodeId {
    g.add_node_with_attrs(
        ...
        vec![
            Attr::new("kind", "uses"),
            Attr::new("resource", uses.resource.clone()),
            Attr::new("arg_count", uses.args.len().to_string()),
        ],
    )
}

fn build_stmt(g: &mut Graph, _proc_id: NodeId, index: usize, stmt: &DirStmt) -> NodeId {
    let (stmt_kind, label) = match stmt {
        DirStmt::Let(s) => ("Let", format!("Let {} = {}", s.name, s.expr)),
        DirStmt::Constrain(s) => ("Constrain", format!("Constrain {}", s.predicate)),
        DirStmt::Prove(s) => ("Prove", format!("Prove {} from {}", s.name, s.from)),
        DirStmt::Effect(s) => ("Effect", format!("Effect {} {}", s.kind, s.payload)),
        DirStmt::Return(s) => ("Return", format!("Return {}", s.expr)),
    };
    ...
    g.add_node_with_attrs(
        ...
        vec![
            Attr::new("kind", "stmt"),
            Attr::new("stmt_kind", stmt_kind),
            Attr::new("index", index.to_string()),
        ],
    )
}

fn build_bind(g: &mut Graph, forge_id: NodeId, bind: &DirBind) -> NodeId {
    let bind_id = g.add_node_with_attrs(
        NodeKind::Bind {
        ...
        vec![
            Attr::new("kind", "bind"),
            Attr::new("source", bind.source.clone()),
            Attr::new("target", bind.target.clone()),
            Attr::new("clause_count", bind.contract.len().to_string()),
        ],
    );
...
fn build_clause(g: &mut Graph, _bind_id: NodeId, clause: &DirClause) -> NodeId {
    g.add_node_with_attrs(
        ...
        vec![
            Attr::new("kind", "clause"),
            Attr::new("key", clause.key.clone()),
            Attr::new("op", clause.op.clone()),
            Attr::new("value", clause.value.clone()),
        ],
    )
}
...
mod tests {
    ...
    #[test]
    fn proc_nodes_include_annotation_attrs() {
        let program = load_fixture();
        let graph = build_dir_graph(&program);
